...
What can go wrong, what stops it, and what does not. Claims point at tests or measurements;
anything unsupported is marked as such.
...
**One caveat.** That $1.5M figure is measured on a fork, where nobody arbitrages the
price back. On a live chain, holding a 6% dislocation on the deepest ETH pool on Base for five
minutes means absorbing every arbitrage trade aimed at it. The measured number is a lower bound; the real cost is unmeasured and much larger.
...
Two details matter for the bound. It binds on **aggregate** size,
...
The cap does not make manipulation impossible, only unprofitable at any allowed series size.
...
more than any series it would void; on a thin pool it is cheap, which is one more reason not to write series on thin pools.
...
1. **Uniswap V3 reports its own history correctly.** The protocol reads a buffer it does not

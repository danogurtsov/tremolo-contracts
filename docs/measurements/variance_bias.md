...
## Method, and one wrong turn worth recording
...
discretisation, not volatility. It reported variance ~50% *below* the grid series, which
initially looked like the grid was inflating variance — the opposite of the truth.
...
**1. The 67% is not noise, it is the expected number.** For a Brownian path, the variance of
...
realized variance.** That is a property of the definition, not a defect — but it is only benign
if everyone pricing the instrument knows it.
...
or be scaled by the measured ratio. This is now a requirement on the pricing layer, recorded in
ADR-0007.
...
- Whether a **stepped** grid (spot at boundaries rather than TWAP) is worth the manipulation
  exposure it would reintroduce. The whole point of TWAP is that moving it is expensive; that
  protection is what costs the third.

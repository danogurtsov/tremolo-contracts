...
interface IPriceObserver {
    ...
    ///      on are exactly the pools whose memory is shortest. Measured cost: 22 244 gas per slot, so a
    /// thousand slots is about $0.33 on Base and buys roughly six more hours on a pool recording every 23
    /// seconds.
    ///
    ///      A no-op for sources that cannot be extended, which is most of them. Callers should
    ///      re-read `maxLookback` afterwards, because extension is not instant: Uniswap grows the buffer
    /// lazily as new observations are written, so capacity bought now becomes history later.

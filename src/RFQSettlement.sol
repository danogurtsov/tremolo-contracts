...
///
///      A separate contract, so the market does not have to know about signatures, nonces or
///      deadlines. Anyone may write a different execution layer against `openImmediate`
///      without touching the part that holds the money.
contract RFQSettlement is EIP712 {
    ...
    /// @dev A maker quotes a size they are willing to take, and takers arrive in pieces;
    ///      tracking filled units rather than a used/unused flag allows partial fills.
    ...
    /// @dev Cancellation is by nonce rather than by quote hash: a maker who wants out

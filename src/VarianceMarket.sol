...
contract VarianceMarket is IVarianceMarket, ERC6909, ReentrancyGuard {
    ...
    ///      Cancelling instead says the true thing.
    ...
    ///
    ///      This exists because manipulation is priced, not prevented. Measured on the live
    ...
    ///      deliberately excluded and which dominates the real cost.
    ///
    ...
    ///
    ///      These exist because of how `eth_estimateGas` interacts with `try/catch`, which is a
    ///      trap rather than a subtlety. The estimator searches for the cheapest gas at which the
    ...
    ///
    ///      This is the other shape: a counterparty who is willing to take a side names the
    ///      strike, and the trade opens against it on the spot. Both legs are funded in the same
    ...
    ///
    ///      It is deliberately permissionless and dumb: it checks parameters and moves
    ///      collateral. Who agreed to what, and on what terms, is settled one layer up in
    ///      `RFQSettlement`, which is where signatures, deadlines and replay protection belong.
    ///
    ...
    /// @dev Two steps on purpose. A single-step transfer hands the pause switch to whatever

...
///
/// @dev Written to find out whether `IPriceObserver` was an abstraction or a description of
///      Uniswap. It was partly the latter, and this adapter is what exposed it. Three things had
///      to change or be conceded:
///
...
///      block. Which means:
///
///      **3. This adapter is materially worse, and says so.** A grid of 24 points costs 24
///      searches, each up to `MAX_SEARCH_STEPS` reads. Feeds update on deviation thresholds, so
///      between updates the series is a step function — genuinely flat, not interpolated — and
...
contract ChainlinkObserver is IPriceObserver {
    ...
    ///      and nothing to ask for, so this does nothing rather than pretending otherwise.
